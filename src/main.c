...
static float get_ebac(const float body_water,
  ...
  float ebac =
    ((0.806 * standard_drinks * 1.2) / (ebac_params.body_water * ebac_params.weight_kgs)) -
...
static void update_text() {
  //update_ebac
  const float ebac = get_ebac(ebac_params.body_water, ebac_params.metabolism,
                              ebac_params.weight_kgs, drinking_state.num_drinks, time_elapsed);
  ...
  if (dp == 0.0) {
    ...
    int dp_h = (int) dp;
    snprintf(countdown_text, sizeof(countdown_text), "%02d H %02d M", dp_h,
             (int) ((dp - dp_h) * 60));
  }
